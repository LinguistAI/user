...
public interface IUserXPRepository extends JpaRepository<UserXP, UUID> {
    ...
    @Query(value = "SELECT r.ranking FROM (SELECT DENSE_RANK() OVER (ORDER BY ux.experience DESC) AS ranking, u.email AS email " +
            "FROM user_xp ux JOIN user u ON u.id = ux.user_id) AS r WHERE r.email = :email",
            nativeQuery = true)
    Long findGlobalUserRankByEmail(String email);

    @Query(value = "SELECT r.ranking FROM (SELECT DENSE_RANK() OVER (ORDER BY ux.experience DESC) AS ranking, ux.user_id as user_id " +
            "FROM user_xp ux " +
            ...
            nativeQuery = true)
    Long findFriendsUserRankByEmail(UUID userId);
